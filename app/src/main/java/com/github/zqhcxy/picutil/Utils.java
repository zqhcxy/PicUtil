...
public class Utils{
    ...
    private int backorfront=0;//默认是后置


    public Utils(Context context,SurfaceView sufv) {
        this(context,sufv,0);
    }
    public Utils(Context context,SurfaceView sufv,int whitch) {
        backorfront= whitch;
        mContext=context;
        this.sv=sufv;
        sh = sv.getHolder();
...
//            releaseCameraAndPreview();
                    if(bCamId !=-  1&&backorfront==0)
                        mCamera=Camera.open(bCamId);
                    else if(fCamId!=-  1&&backorfront==1){
...
//            Toast.makeText(getApplicationContext(), "宽:"+prvw+"高:"+prvh, Toast.LENGTH_LONG).show();
    ...
    public void tackPic() {
        if (mCamera != null) {//mShutterCallback
            try {
